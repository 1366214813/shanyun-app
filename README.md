...
服装店库存管理工具，支持 OCR 拍照识别小票/标签批量入库，蓝牙连接汉印T260打印吊牌，Supabase 云端同步，每日自动备份。
...
- **数据管理** — 导出 Excel 表格（商品/客户/订单，自动保存到 Documents）、JSON/Excel 数据导入（追加模式，自动识别格式，校验异常数据）、一键清空数据（需输入 CLEAR 二次确认）、每日自动备份（每天第一次打开时自动备份到 Documents）
...
│       ├── exportData.ts       # Excel 导出（保存到 Documents + Share 分享）
│       └── autoBackup.ts       # 每日自动备份（每天第一次打开时自动备份）
├── assets/
│   └── models/             # OCR 模型文件
...
- 支持导出 Excel 表格（商品/客户/订单三个工作表，自动保存到 Documents）
- 支持 JSON/Excel 数据导入（追加模式，自动识别格式，校验异常数据）
- 支持一键清空数据（需输入 CLEAR 二次确认）
- 支持每日自动备份（每天第一次打开时自动备份到 Documents，可开关）
