...
- **首页仪表盘** — 快捷操作按钮（开单/客户/商品/打印）、今日销售/利润/订单数/库存总量卡片（可点击跳转）、7日趋势图、最近订单、高价值客户、库存预警
...
- **吊牌打印** — 蓝牙连接汉印T260热敏打印机（HM-T260LR），经典蓝牙 SPP (RFCOMM) + ESC/POS `GS v 0` 位图协议 (ESC_POLI 模式)，BLE 作为后备通道（iOS 走 BLE），电量检测
